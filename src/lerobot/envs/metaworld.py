...
class MetaworldEnv(gym.Env):
    ...
    def _render_camera(self, camera_name: str) -> np.ndarray:
        ...
        old_camera_name = getattr(renderer, "camera_name", None)
        ...
        if camera_id < 0:
            names = [
                mujoco.mj_id2name(self._env.model, mujoco.mjtObj.mjOBJ_CAMERA, i)
                for i in range(self._env.model.ncam)
            ]
            raise ValueError(f"Unknown camera {camera_name}. Available cameras: {names}")

        renderer.camera_id = camera_id
        renderer.camera_name = camera_name

        image = renderer.render(render_mode="rgb_array")

        if old_camera_id is not None:
            renderer.camera_id = old_camera_id
        if old_camera_name is not None:
            renderer.camera_name = old_camera_name

        if image is None:
            raise RuntimeError(f"Render returned None for camera: {camera_name}")
    ...
    def _format_raw_obs(self, raw_obs: np.ndarray) -> dict[str, Any]:
        ...
        images = {cam: self._render_camera(cam).copy() for cam in self.camera_names}
